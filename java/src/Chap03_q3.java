...
public class Chap03_q3 {
    public static void main(String[] args) {
        ...
        for(int p=0;p<product_names.length;p++)
        {
            ...
            System.out.printf("상품 %s의 총 판매 수량 = %d, 평균 판매 수량 = %.2f\n",
                product_names[p], yearly_sells[p], (double)yearly_sells[p] / 12);
        }
        ...
        System.out.printf("연간 판매 수량이 가장 높은 상품 : %s (총 판매 수량: %d)\n\n", 
            product_names[max_sells_idx], max_sells);
        
        System.out.print("총 판매 수량이 알고 싶은 월을 입력하세요 (1~12): ");
        int select_month = s.nextInt();

        int monthly_sells = 0;
        for(int p=0;p<product_names.length;p++)
        {
            monthly_sells += product_sells[select_month-1][p];
        }
